...
s[9]='\0';
FILE *f=fopen(s,"w");
char *code="#include<stdio.h>%c#include<stdlib.h>%c#include<string.h>%cint main(){%cint i=%d;%cif(--i>=0){%cchar c=i+'0';%cchar *cmd=malloc(sizeof(char)*47);%cchar *s=malloc(sizeof(char)*10);%cstrncpy(s,%cSully_x.c%c,9);%cs[6]=c;%cs[9]='%c0';%cFILE *f=fopen(s,%cw%c);%cchar *code=%c%s%c;%cfprintf(f,code,10,10,10,10,i,10,10,10,10,10,34,34,10,10,92,10,34,34,10,34,code,34,10,10,34,34,10,10,10,92,10,10,10,10);%cstrncpy(cmd,%cgcc -Wall -Werror -Wextra Sully_x.c -o Sully_x%c,45);%ccmd[45]=c;%ccmd[32]=c;%ccmd[46]='%c0';%csystem(cmd);%cfree(s);%cfree(cmd);}}%c";
fprintf(f,code,10,10,10,10,i,10,10,10,10,10,34,34,10,10,92,10,34,34,10,34,code,34,10,10,34,34,10,10,10,92,10,10,10,10);
